...
public class PayServiceImpl implements IPayService {
    ...
    public Result createPayment(TradePay tradePay) {
        ...
        if (StringUtils.isEmpty(tradePay) || StringUtils.isEmpty(tradePay.getOrderId())) {
    ...
    public Result callbackPayment(TradePay tradePay) {
        ...
        if (tradePay.getIsPaid().intValue() == ShopCode.SHOP_ORDER_PAY_STATUS_IS_PAY.getCode()) {
            ...
            TradePay tradePay1 = tradePayMapper.selectByPrimaryKey(payId);
            //判断支付订单是否存在
            if (StringUtils.isEmpty(tradePay1)) {
                CastException.cast(ShopCode.SHOP_PAYMENT_NOT_FOUND);
            }
            tradePay1.setIsPaid(ShopCode.SHOP_ORDER_PAY_STATUS_IS_PAY.getCode());
            int i = tradePayMapper.updateByPrimaryKey(tradePay1);
            ...
            if (i == 1) {
                ...
                try {
                    ...
                    threadPoolTaskExecutor.submit(new Runnable() {
                        ...
                        public void run() {
                            try {
                                SendResult sendResult = sendMessage(topic, tag, String.valueOf(tradePay.getPayId()), JSON.toJSONString(tradePay));
                                if (sendResult.getSendStatus().equals(SendStatus.SEND_OK)) {
                                    log.info("消息发送成功");
                                    //等待发送结果，如果mq接受到消息，删除发送成功的消息
                                    tradeMqProducerTempMapper.deleteByPrimaryKey(tradeMqProducerTemp.getId());
                                    log.info("持久化到数据库的支付成功的消息已删除");
                                }
                            } catch (Exception e) {
                                e.printStackTrace();
                            }
