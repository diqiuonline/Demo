...
<div style="width:380px;margin:0 auto;margin-top:50px;">
    <ul class="pagination" style="text-align:center; margin-top:10px;">
        <li <c:if test="${pageBean.currPage==1}">class='disabled'</c:if>>
            <a href="${ pageContext.request.contextPath }/ProductServlet?method=findByCid&currPage=${pageBean.currPage-1}&cid=${
            param.cid}" aria-label="Previous"><span aria-hidden="true">&laquo;</span></a>
        </li>
        <c:forEach var="i" begin="1" end="${ pageBean.totoPage }">
            ...
            <c:if test="${ pageBean.currPage != i }">
                <li><a href="${ pageContext.request.contextPath }/ProductServlet?method=findByCid&currPage=${i}&cid=${ param.cid}">
                        ${ i }</a></li>
            </c:if>

        </c:forEach>

        <li <c:if test="${ pageBean.currPage == pageBean.totoPage }">class='disabled'</c:if>>
            <a href="${ pageContext.request.contextPath }/ProductServlet?method=findByCid&currPage=${
            pageBean.currPage+1 }&cid=${ param.cid}" aria-label="Next">
...
<div style="width:1210px;margin:0 auto; padding: 0 9px;border: 1px solid #ddd;border-top: 2px solid #999;height: 246px;">
    ...
    <div style="float: right"><a href="${pageContext.request.contextPath}/ProductServlet?method=delCook">清除浏览记录</a></div>
