...
public abstract class BaseAction<T>  extends ActionSupport implements ModelDriven<T> {
    ...
    public void PagetoMapToJson(Page<T> pageData) throws IOException {
        ...
        String string = JSONObject.toJSONStringWithDateFormat(result,"yyyy-MM-dd HH:mm:ss.SSS");
    ...
    public void pageBeanToJson(PageBean<T>pageBean) throws IOException {
        ServletActionContext.getResponse().setContentType("text/html;charset=utf-8");
        String string = JSONObject.toJSONString(pageBean);
        ServletActionContext.getResponse().getWriter().println(string);
    }
    //弹栈式转换json
    protected void PushPageData(Page<TransitInfo> pageData) {
        Map<String, Object> result = new HashMap<String, Object>();
        result.put("total", pageData.getTotalElements());
        result.put("rows", pageData.getContent());
