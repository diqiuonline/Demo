...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //开始学习 获取k课程计划所对应的视频地址
      study(chapter){
        courseApi.get_media(this.courseId,chapter).then(res =>{
          if (res.success) {
            //操作成功 获取视频播放地址
            let fileUrl = res.fileUrl; //相对路劲
            let videoUrl = sysConfig.videoUrl + fileUrl; //视频完整播放路径
    ...
    created(){
      ...
      systemApi.course_view(this.courseId).then((view_course) => {
        ...
        //如果该this.chapter不等与0 直接播放该课程计划对应的视频
        if (this.chapter != '0') {
          //获取该课程计划对应的视频
          this.study(this.chapter)
        } else {
          //找到该课程的二级课程计划第一个课程计划id，取出该课程计划对应的视频
